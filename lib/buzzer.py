...
# Set up pin PWM timer for output to buzzer_pinzer or speaker
buzzer_pin = Pin("P23") # Pin 23 with timer 8 Channel 2
timer = PWM(0, frequency=0)
channel = timer.channel(2, duty_cycle=0.5, pin=buzzer_pin)
...
# Mario Bros tone example
# source from here http://www.linuxcircle.com/2013/03/31/playing-mario-bros-tune-with-arduino-and-piezo-buzzer_pinzer/
...
def play_all_notes():
    for i in note:
       print(i)
       timer=PWM(0, frequency=i)
       channel.duty_cycle(0.30)
       time.sleep(0.3)
